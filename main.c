...
void toMove(){
    ...
    do{
        ...
        if(headSnake.direction==RIGHT) Right();
        else if(headSnake.direction==LEFT) Left();
        else if(headSnake.direction==DOWN) Down();
        else if(headSnake.direction==UP) Up();
    ...
    // bend increment
    if(
    (key==RIGHT&&headSnake.direction!=LEFT&&headSnake.direction!=RIGHT)||
    (key==LEFT&&headSnake.direction!=RIGHT&&headSnake.direction!=LEFT)||
    (key==UP&&headSnake.direction!=DOWN&&headSnake.direction!=UP)||
    (key==DOWN&&headSnake.direction!=UP&&headSnake.direction!=DOWN)){
        ...
        //It moves the snack
        if(key==UP) headSnake.y--;
        if(key==DOWN) headSnake.y++;
        if(key==RIGHT) headSnake.x++;
        if(key==LEFT) headSnake.x--;
